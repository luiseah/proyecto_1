...
final class Route
{
    ...
    {
        if (!$this->hooked) {
            add_filter('init', [__class__, 'onInit'], 1, 0);
            $this->hooked = true;
        }
    }
...
//            $this->getPathVariables($route),
     ...
     * @return string
     */
    private function getControllerClass($route)
    {
        return $route->callable[0];
    }

    private function getControllerMethod($route)
    {
        return $route->callable[1];
    }
    ...
    private function handle()
    {
        ...
        if (is_array($route->callable)) {
            $route->callable = [
                plugin()->make($this->getControllerClass($route)),
                $this->getControllerMethod($route)
            ];
        }
        ...
        if (isset($route->args['redirect'])) {
            $redirect = $route->args['redirect'];
    ...
    private function getParams()
    {
        if (!isset(self::$rParams)) {
            self::$rParams = array_map(
                function ($value) {
                    return filter_var($value, FILTER_SANITIZE_FULL_SPECIAL_CHARS, ['flags' => FILTER_FLAG_STRIP_BACKTICK]);
                },
                $_GET
            );
        }
